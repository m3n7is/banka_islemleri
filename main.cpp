...
int main(){
	...
	struct ozet kayit[10]; // Main'in dýþýnda tanýmladýðýmýz struct ozet adlý bilgi deposu için bir deðiþken dizisi tanýmlýyoruz.
	int hesapno,tmp1=1,i=0; // Hesap numarasý, tuþlama girdisi, özet kaydý sayacý tanýmlýyoruz.
	double bakiye; // Hesap bakiyesi tanýnlýyoruz.
	cout << "Kayýtlý hesap bulunamadý. Lütfen hesap oluþturunuz" << endl; // Giriþte hesap bulunmadýðýný belirtiyoruz.
	...
	while(tmp1 !=0){ // Ýþlem seçeneklerimizi baþlatýyoruz. 0 -> Çýkýþ iþlemi.
		...
		if(tmp1 == 1){ // Yeni hesap oluþturuyoruz.
			...
			getline (cin,isim); // Ýsim alÄ±yoruz. BoÞluk saymasý için getline kullanýyoruz.
			cout << "Bakiye: ";
			cin >> bakiye;
			cout << "Hesap Numarasý: ";
			cin >> hesapno;
			...
			if(isim == "") // Zaten hesap yoksa hata veriyoruz.
				...
				isim=""; // Ýsmi boþaltarak hesaba silinmiþ süsü veriyoruz. Diðer bilgileride sýfýrlýyoruz.
				bakiye=0;
				hesapno=0;
			...
			if(isim == "") // Zaten hesap yoksa hata veriyoruz.
				...
				hesap.bakiyesor(isim); // Hesap sýnýfýmýzýn bakiye sorgulama iþlevini gören fonksiyonunu çaðýrýyoruz.
		}
			...
			if(isim == "") // Zaten hesap yoksa hata veriyoruz.
				...
				for(;i>0; i--) // Hareketleri bir dizide tuttuðumuz için döngü kullanarak dizideki hareketleri listeliyoruz.
			...
			cout << "Ýyi Günler! ";
		else // Listede olmayan iþlem tutarý için hatalý tuþlama hatasý veriyoruz.
			cout << "\nHatalý Tuþlama." << endl;
	}
	return 0;
}
